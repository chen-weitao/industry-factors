...
# Fama-MacBeth回归
fpmg <- pmg(excess_m_r~mv_within + mv_across + bm_within + bm_across + p12m_within + p12m_across, data_filter_last, index=c("type","V1"))
...
a.formula <- excess_m_r~ mv_within + mv_across + bm_within + bm_across + p12m_within + p12m_across
...
first.step.df$mv_dif <- first.step.df$mv_within-first.step.df$mv_across
first.step.df$bm_dif <- first.step.df$bm_within-first.step.df$bm_across
first.step.df$p12m_dif <- first.step.df$p12m_within-first.step.df$p12m_across
...
mv_across <- coeftest(lm(first.step.df$mv_across ~ 1), vcov = NeweyWest(lm(first.step.df$bm_within ~ 1), lag = lag_num, prewhite = FALSE))
p_zhi <- rbind(p_zhi,mv_across)
...
bm_across <- coeftest(lm(first.step.df$bm_across ~ 1), vcov = NeweyWest(lm(first.step.df$bm_within ~ 1), lag = lag_num, prewhite = FALSE))
p_zhi <- rbind(p_zhi,bm_across)
...
p12m_across <- coeftest(lm(first.step.df$p12m_across ~ 1), vcov = NeweyWest(lm(first.step.df$bm_within ~ 1), lag = lag_num, prewhite = FALSE))
p_zhi <- rbind(p_zhi,p12m_across)
